...
window.updateCartTable=function(){
  ...
  cart.forEach((c,i)=>{
    ...
    tr.innerHTML=`
      ...
      <td>
        <button class="cart-btn" onclick="changeQty(${i},1)">+</button>
        <button class="cart-btn" onclick="changeQty(${i},-1)">-</button>
      </td>`;
    tbody.appendChild(tr);
    total += c.price*c.qty;
...
window.checkout=async function(method){
  ...
  for(let c of cart){
    ...
    const saleId = "V" + Date.now() + Math.floor(Math.random()*1000);
  ...
  loadStockList();
  loadSalesTable();
  printTicket(salesToday.slice(-1)); // imprime la última venta
};

function printTicket(sales){
  const printWindow=window.open('','Print','width=800,height=600');
  let html='<h2>SUPERMERCADO X - TICKET</h2><hr>';
  sales.forEach(p=>{
    html+=`${p.name} [${p.qty}] ($${p.price.toFixed(2)})<br>`;
  });
  html+=`Total: $${sales.reduce((a,b)=>a+a+b.price*b.qty,0).toFixed(2)}`;
...
window.deleteSale=async function(id){
  ...
  snapshot.forEach(async d=>{
    ...
    const deletedId = "D" + Date.now() + Math.floor(Math.random()*1000);
    deletedSales.push({...saleData, reason, id:deletedId});
...
window.printDailyReport=function(){
  let html="<h2>REPORTE Z</h2>";
  html+=salesToday.map(s=>`${s.id} - ${s.name} [${s.qty}] ($${s.price})`).join("<br>");
