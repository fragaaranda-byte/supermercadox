...
window.loadSalesTable = async function(){
  ...
  snapshot.forEach(docSnap=>{
    ...
    const tr = document.createElement("tr");
    tr.innerHTML = `<td>${new Date(s.date.seconds*1000).toLocaleString()}</td>
...
window.printDailyReport = function(){
  ...
  deletedSales.forEach(p=>{
    html+=`${p.name} [${p.qty}] ($${p.price.toFixed(2)}) - Motivo: ${p.reason}<br>`;
  });
