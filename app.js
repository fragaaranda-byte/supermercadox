...
window.addStock = async function(){
  ...
  if(!snapshot.empty){
    ...
    await addDoc(collection(db,"products"),{
      code,name:name.toLowerCase(),price:totalPrice,currentStock:qty
    });
...
window.loadProductToModify= async function(){
  ...
  docSnap.forEach(d=>{
    ...
    document.getElementById("modCents").value=cents;
...
document.getElementById("btnModificar").addEventListener("click", async ()=>{
  ...
  const modPrice=parseInt(document.getElementById("modPrecio").value)||0;
  const modCents=parseInt(document.getElementById("modCents").value)||0;
...
window.updateCartTable=function(){
  ...
  cart.forEach((c,index)=>{
    ...
    <td><button onclick="changeQty(${index},1)">+</button> <button onclick="changeQty(${index},-1)">-</button></td>`;
    tbody.appendChild(tr);
    total+=c.price*c.qty;
  });
